...
use super::{ResponsesRequest, responses_error, responses_to_anthropic_for_compaction};
/// 仅测试用：确认带 checkpoint 文案但没有 trigger 的请求仍走普通生成路径。
#[cfg(test)]
...
fn prepare_request(
    ...
    let (mut anthropic_req, _) = responses_to_anthropic_for_compaction(req, metadata)?;
...
mod tests {
    ...
    fn translate_compact_input(mut request: ResponsesRequest) -> MessagesRequest {
        ...
        responses_to_anthropic_for_compaction(request, None)
            .unwrap()
            .0
    }
    ...
    fn retry_truncation_preserves_tool_item_pairing_and_summary_turn() {
        ...
        let mut anthropic = responses_to_anthropic_for_compaction(request, None).unwrap().0;
