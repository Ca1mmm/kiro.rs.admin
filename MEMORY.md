...
## 工具调用配对校验（fail closed）

- 本仓库在 Anthropic / Responses 入口都做严格配对校验，不再像上游那样静默删除
  孤立的 `tool_use` / `tool_result`：删掉一侧会改写对话，坏结果还会在客户端重试时
  变成未经校验的历史结果。
- `converter.rs`：`ConversionPurpose::Generate` 走 `validate_tool_pairing_strict`；
  上游用于压缩的 `validate_compaction_tool_sequence` 保留在 Compact 分支。
  `validate_tool_pairing` / `remove_orphaned_tool_uses` 只在 `#[cfg(test)]` 下保留。
- `responses.rs`：`validate_tool_replay` 默认 `ReplayPolicy::Strict`；Codex 远端压缩
  （`compaction_trigger`）走 `responses_to_anthropic_for_compaction`，允许输入末尾
  那一串未闭合的 `function_call` —— 它们会由 `prepare_summary_turn` 补一条
  cancelled 结果，放行后配对依然完整。
